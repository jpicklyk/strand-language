...
- **Familiar-shaped authoring surface.** Defined by [Q-061](open-questions.md#Q-061) and [`proposals/familiar-surface-lowering.md`](proposals/familiar-surface-lowering.md). Layer F, a fully typed familiar-syntax dialect lowering mechanically to the canonical graph with cross-surface hash equality against Layer A as the correctness property; the Q-021 A/B against Layer A and the conventional baselines is the exit criterion in either direction. Step 1 (the pure subset plus effects and attempt) landed 2026-06-13; the machine, schema, handler, and capability forms (step 2) and the A/B measurement (step 3) remain.

## Tier 2 — Expressiveness ceilings that make the measurement representative

First-pass correctness is measured over a task suite; the suite is only as representative as the programs Strand can express. The module-system ceiling is closed ([Q-043](open-questions.md#Q-043)), as are streaming I/O ([Q-045](open-questions.md#Q-045)), the actor-runtime stream bridge ([Q-046](open-questions.md#Q-046)), in-language error recovery ([Q-048](open-questions.md#Q-048), N-047 Attempt), and nested recursive types at value-construction sites ([Q-053](open-questions.md#Q-053), N-048 RecursiveProjection).

- **Runtime schema enforcement extensions (Layer 7 step 2 continuation).** Defined by the deferred scope of [Q-047](open-questions.md#Q-047) and [`proposals/implemented/runtime-schema-enforcement.md`](proposals/implemented/runtime-schema-enforcement.md) (the foundational slice landed 2026-06-03): ForeignNode-backed checkers (trust model via Q-006), the HTML5/SVG/PDF blessed-library set (HTML5 and SVG now constructible on N-048 RecursiveProjection per [Q-053](open-questions.md#Q-053), not yet built), bytecode-VM enforcement parity (a `CHECK_SCHEMA` lowering), and schema-typed state-machine validation.
- **Layer A grammar parity.** Defined by [Q-057](open-questions.md#Q-057). Five implemented surfaces (module manifests, the EventStream source edge, Sample overflow policy, external-hash NodeRefs, and N-048 RecursiveProjection) are unauthorable in Layer A and therefore unreachable through the documented agent interface; the suite cannot exercise what agents cannot author. Effect projections and RecursiveSelf depth, originally on this list, became authorable in the 2026-06-13 density-v5 and Json-migration work.

...
- **Nested recursive types at value-construction sites.** Defined by [Q-053](open-questions.md#Q-053). Composite recursive structures (`μX. ... List<X> ...`) cannot be constructed directly; the splice workaround loses type precision. Blocks the HTML5/SVG blessed libraries and any agent-authored AST or tree-of-lists shape. Its fix's encoding budget rides epoch 2 per [Q-062](open-questions.md#Q-062) — designed on its merits, not around hash invariance.
...
- **Embeddable runtime and per-instance host policy.** Defined by [Q-054](open-questions.md#Q-054); **resolved at the facade level** (2026-06-13, [`proposals/implemented/embeddable-runtime.md`](proposals/implemented/embeddable-runtime.md)). A `HostPolicy` value object and a `StrandRuntime` facade ship as the published embedding surface, with the CLI a thin client and the install/restore of the `Builtins` singletons centralized behind the facade. The remaining work under this identifier is the value-threading follow-up: removing the bare-name singleton reads from the builtin lambdas so two runs are concurrently isolated in one JVM without the install serialization (plus `CredentialScrubber` snapshot/restore and per-machine policy within a group). The facade is the structural prerequisite for any server, daemon, or multi-tenant host, and shapes the Rust VM's host API (Q-017 step 2).

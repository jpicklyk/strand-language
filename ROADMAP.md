...
First-pass correctness is measured over a task suite; the suite is only as representative as the programs Strand can express. The module-system ceiling is closed ([Q-043](open-questions.md#Q-043)); two remain.

- **Streaming and asynchronous I/O.** Defined by [Q-045](open-questions.md#Q-045) and [`proposals/streaming-async-io.md`](proposals/streaming-async-io.md). A uniform synchronous streaming-handle contract on the existing `ResourceTable` pattern: an open builtin returns a resource, `<Namespace>.Stream.Receive(handle, maxBytes)` drains chunk-by-chunk, `<Namespace>.Stream.Close` releases it. Core slice is per-provider LLM streaming variants plus an `Option`-EOF `Net.Stream.Receive`. The proposal is drafted; no implementation has landed. The actor-runtime bridge, SSE decoding, process and file streaming, streaming send, and TLS/WebSocket are deferred within the proposal.
- **Runtime schema enforcement (Layer 7 step 2).** Defined by the deferred scope of [Q-035](open-questions.md#Q-035) and [`proposals/implemented/schema-and-invariant.md`](proposals/implemented/schema-and-invariant.md), with [`design/rendering-and-views.md`](design/rendering-and-views.md) and ADR-009. Lifts Schema and Invariant evaluation from verify-time on statically-known values to runtime on dynamic values, adds ForeignNode-backed checkers, and extends the blessed-library set (HTML5, SVG, PDF; HTML5 is blocked on the nested-recursive-self pattern). No dedicated open question is registered yet; one should be opened on pickup.

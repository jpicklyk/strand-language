...
**Status:** Wave 3 draft
**Last revised:** 2026-06-13 (N-048 RecursiveProjection added to the § Type system inventory — closes the Q-053 nested-recursive value-construction gap per [`proposals/implemented/nested-recursive-types.md`](../proposals/implemented/nested-recursive-types.md). A value-construction site whose type is a component of a nested recursive type (`μX. … List<X> …`) names a `RecursiveProjection(recursiveType: the closed outer μ, path: an ordered Case/Field/Unfold selector list)` rather than the bare open inner μ; the verifier enters the outer μ's binder context before walking the path so an inner `RecursiveSelf(depth=N)` resolves, with the N-042 depth field unchanged. Closed-target discipline (the analogue of NodeRefTargetMustBeClosed) gives the projection one canonical hash. Landed additively under encoding epoch 1 — canonical tag 48 is a fresh category, so no existing program hash moved.) 2026-06-11 (N-047 Attempt added to the control-flow inventory — in-language error recovery per Q-048; evaluating `Attempt(body)` reifies a catchable runtime failure as an `Err` value while uncatchable defects and host-policy stops propagate; full semantics, the catchable-failure taxonomy, and both-backend implementation notes in `proposals/implemented/error-recovery.md`. The 2026-06-10 revision in the same series corrected § Types to the implemented strict-structural-equality discipline (the subtyping decision is tracked as Q-049), refreshed the stale N-001–N-031 inventory references, and added the § Hash construction pointer to `canonical-encoding.md`.) 2026-06-03 (N-028 EventStream gains an optional `source`(0..1) → Expression edge for the Q-046 actor-runtime bridge — it binds an `external` stream to an IO-opening `Application` (`*.CreateStream` / `Net.Connect`) so a host-side feeder drains the opened stream's chunks into the consuming machine as events, inheriting the existing `OverflowPolicy` / `EventRecorder` replay. No new node category. The edge is omitted from the canonical encoding when absent, so every pre-Q-046 EventStream hashes byte-identically; a source-bound stream encodes its source as a trailing hash reference, collision-free against the slice-3.1/3.6 optional fields. Implemented in the Kotlin/JVM reference implementation per [`proposals/implemented/actor-runtime-stream-bridge.md`](../proposals/implemented/actor-runtime-stream-bridge.md); the verifier enforces that `source` resolves to a registered opener declaring its semantic effect, the `eventType` is `Bytes`, the stream is `external`, and the overflow policy is `BlockProducer`, while the runtime gates group start on the opener's transport effects ∪ `Network.Receive` being covered by the group's capabilities.) 2026-05-28 (N-046 ModuleManifest added under a new "Composition and distribution" subsection — a content-addressed grouping primitive bundling exports with aggregate effect declarations per [Q-043](../open-questions.md#Q-043) and [`proposals/cross-store-federation.md`](../proposals/cross-store-federation.md). Each ManifestExport carries `target: Hash`, `declaredEffects: List<EffectCategory>`, and `displayName: Name` (metadata, hash-excluded). The verifier admits a manifest by checking exact equality between each export's declaredEffects and the effect surface of its target (the function's declared effect row for a function export; the construction closure for a value export) — under-declaration rejected for safety, over-declaration rejected for precision. The manifest is informational/declaratory: consumers reference exports by hash directly via N-019 NodeRef; the manifest has no runtime evaluation semantics. ADR-003 was amended the same day to clarify its conclusion: it rejects name-based identity, import-path resolution, visibility, and version constraints — not content-addressed grouping primitives. The N-019 NodeRef row gains a one-sentence note about resolution through the federation chain. This is Layer 2 step 3 in the implementation milestone plan; N-046 ModuleManifest shipped as step 3b in the Kotlin/JVM reference implementation on 2026-05-28 (the federation runtime, step 3a, remains pending). The implementation clarified "closure of the target" to "effect surface": a bare Lambda's closure is empty in Strand's model — effects release at call sites — so a function export's certified surface is its declared effect row, not the closure of the Lambda value.) 2026-05-26 (N-045 ResponseSchemaSpec added to the "Agent-native capabilities" subsection — symmetric Schema-bearing wrapper for the `GenerateRequest.responseSchema` field, completing the pair started by N-044 ToolDef. The wrapper has a single `schema → Schema` edge, no metadata content fields; the verifier statically projects the Schema's `valueType` to JSON Schema at admission (`ResponseSchemaTypeUnsupported` on rejection). Both N-044 ToolDef and N-045 ResponseSchemaSpec now cover the two Schema-bearing positions on `GenerateRequest` through structural graph edges rather than the `Option<JsonValue>` expedient.) 2026-05-26 (N-044 ToolDef added under a new "Agent-native capabilities" subsection — promotes the Q-037 Phase 1 ToolDef from a runtime `ProductV` convention to a structural graph node per the original proposal § 3.8 design. Closes the Q-037 deviation #1 partial `ToolParamTypeUnsupported` verifier walk: the rule now fires on every static ToolDef by construction. The `parameterSchema` edge points at an N-032 Schema, bringing tool parameters into Strand's structural-reasoning machinery (Schema + SchemaChecker + JsonSchemaProjection). `name` and `description` are metadata-only and excluded from the canonical encoding, matching ParameterDecl.name treatment.) 2026-05-24 (N-032 Schema and N-033 Invariant inventory rows added under a new "Structured outputs" subsection — slots were pre-reserved by `rendering-and-views.md` since Wave 3 design and are now implemented in the Kotlin/JVM reference implementation per `proposals/implemented/schema-and-invariant.md`. Full edge schemas, well-formedness rules, and canonical encoding remain documented in `rendering-and-views.md` § Schema mechanism; the inventory rows here are minimal pointers. One implementation deviation worth noting: `Invariant.targetSchema` is excluded from canonical encoding to break the Schema↔Invariant hash cycle.) 2026-05-23 (N-034 TypeAbstraction and N-035 ForallType added; N-013, N-014, N-016 revised; § Hash construction clarified — `fieldName`, `caseName`, `categoryName`, and `streamKind` re-marked as structural identifiers rather than metadata, and the metadata-exclusion rule restated to depend on edge role rather than target category; N-036 CapabilityScope added — formalizes the capability-narrowing node described in `effects-and-capabilities.md` § Capability mechanism; N-020 ForeignNode entry extended with the explicit `target` structural content field that ADR-005 describes but the original inventory left implicit, and the `binding` Provenance edge marked optional; N-018 VarRef and § Variables and binding revised to recognize Pattern.VariablePattern as a binding occurrence; N-037 ProductValue, N-038 ProductFieldValue, N-039 ProductFieldGet, and N-040 SumValue added — closes the value-construction gap for product and sum types declared by N-008/N-009/N-010/N-011. The N-025 Pattern category gains a constructor variant for matching against sum cases, with arbitrary nested patterns over the payload; N-041 RecursiveType and N-042 RecursiveSelf added — closes the recursive-type gap via μ-binder positional encoding. § Well-formedness "Termination of typing" rule updated to name RecursiveType as the proper mechanism for recursive types; N-043 Handler added — formalizes the no-continuation effect handler described in `effects-and-capabilities.md` § Effect handlers, with the closure-subtraction rule that makes it the only node category that removes effects)
...
| N-048 | RecursiveProjection | `recursiveType`(1) → RecursiveType, `path`(1) → ProjectionPath (content field) | A type expression denoting a component *position* inside the closed enclosing recursive type `recursiveType`, resolved by entering that μ's binder context before walking `path`. `path` is a non-empty ordered list of selector steps — `Case(caseName)` (after one unfold to a Sum, select the case payload), `Field(fieldName)` (select a Product field's type), `Unfold` (enter a nested μ, raising the live recursive-binder depth so an inner `RecursiveSelf` resolves). RecursiveProjection appears only in type position. It is closed iff `recursiveType` is a self-contained μ; the verifier rejects an open target (`RecursiveProjectionTargetNotClosed`, the type-position analogue of NodeRefTargetMustBeClosed), a non-recursive target (`RecursiveProjectionTargetNotRecursive`), and a path that cannot be walked (`RecursiveProjectionPathStepMismatch` / `…CaseNotFound` / `…FieldNotFound` / `…PathSelectsNullaryCase`). |

A value whose type is a *nested* recursive type — a `μX. … List<X> …` shape, where an inner μ (such as a list) carries elements of the *outer* μ — cannot name that inner type directly at a value-construction site. The inner μ has no standalone meaning: its `RecursiveSelf` reaching the outer binder is unresolvable in isolation, exactly the property that makes a bare `VarRef` standalone-meaningless. RecursiveProjection (N-048) is the construction-site front-door: a value-construction `ofType` (or `paramType`, field type, pattern type) targeting such a component points at a `RecursiveProjection` of the closed outer μ selecting the position being built, rather than at the bare open inner μ. Because the projection's only type edge is the closed outer μ, it has one canonical hash and one canonical interpretation independent of where it is referenced — the same closed-anchor discipline the equirecursive `fold` site uses (the introduction form always supplies the value as an inhabitant of the named recursive type, never of an open inner sub-term). The N-042 depth field is unchanged; RecursiveProjection composes with it — the projection supplies the binder context, the depth resolves within it.
...
| ID | Category | Edges | Notes |
|----|----------|-------|-------|
| N-019 | NodeRef | `target`(1) → any (by hash) | Reference to another graph node by hash; allows references that cross module-like boundaries. Resolution may proceed through a federation chain of `NodeResolver` over multiple HashStores ([Q-043](../open-questions.md#Q-043)) — the canonical encoding is unchanged whether the target is local or fetched. |
...
### Control flow (N-023 through N-026, N-047)
...
| N-047 | Attempt | `body`(1) → Expression | Observes the evaluation of `body` as a value. Produces `Ok(v)` when the body yields `v`; produces `Err({kind, detail})` when the body raises a catchable runtime failure. The result type is the structural sum `Ok(T) \| Err(ErrorPayload)` synthesized from the body's inferred type. Introduces no binders; no content fields. |
...
The Attempt node (N-047) is the error-recovery mechanism, resolving [Q-048](../open-questions.md#Q-048). Evaluating `Attempt(body)` evaluates the body; a value `v` becomes `Ok(v)`, and a *catchable* runtime failure becomes `Err(ErrorPayload)` where `ErrorPayload` is the fixed structural product `{kind: String, detail: String}` — `kind` a stable machine-branchable vocabulary, `detail` a scrubbed diagnostic. The catchable set is failures of the world: IO failures and runtime schema-invariant violations. Program defects (a missed match case, a non-callable in call position) and host-policy stops (capability or refinement denial, sandbox denial, resource exhaustion) are not observable in-language and propagate through any number of enclosing Attempts. The effect closure of an Attempt is the closure of its body — failures are not effects, so the statically computed harm bound is unchanged by the node's presence. The recovery logic is an ordinary Match over the synthesized sum; because types are structural and content-addressed, the sum an author declares for its patterns is the same node as the one the verifier synthesizes. The complete catchable-failure taxonomy, the budget interaction (a catch does not reset evaluation counters), and the evaluator semantics for both backends are specified in [`proposals/implemented/error-recovery.md`](../proposals/implemented/error-recovery.md).
...
| N-028 | EventStream | `eventType`(1) → Type, `streamKind`(1) → Name (structural; values: `external`, `internal`, `output`), `source`(0..1) → Expression | An event stream node; sources and sinks are distinguished by `streamKind`. The `streamKind` is structural — it determines the stream's role in the system, so an external and an internal stream over the same event type are distinct nodes that hash differently. The optional `source` edge (Q-046) binds an `external` stream to an IO source — an `Application` of an IO-opening builtin (`*.CreateStream` / `Net.Connect`) — so the runtime feeds the opened stream's chunks into the machine as events; it is omitted from the canonical encoding when absent, so a stream without a `source` hashes identically to its pre-Q-046 form. See [state-machines.md](state-machines.md) and [`proposals/implemented/actor-runtime-stream-bridge.md`](../proposals/implemented/actor-runtime-stream-bridge.md). |
...
| ID | Category | Edges | Notes |
|----|----------|-------|-------|
| N-046 | ModuleManifest | `export`+ → ManifestExport (positional), `description`(?) → Provenance (metadata), `manifestSignature`(?) → Bytes (metadata) | A content-addressed grouping primitive that bundles a set of exported hashes with aggregate effect declarations. Each ManifestExport is a triple `(target: Hash, declaredEffects: List<EffectCategory>, displayName: Name)`. The verifier admits a manifest by checking, for every export, that `declaredEffects` *exactly* equals the export's effect surface — for a function export (the common case) the function's declared effect row, for a value export the construction closure — with under-declaration rejected (`ManifestExportEffectMismatch`) for safety and over-declaration rejected for precision. A bare Lambda's closure is empty (effects release at call sites, not at closure construction), so the certified surface for a function export is its effect row — what a consumer incurs by calling the export — rather than the closure of the Lambda value. `displayName` is metadata content excluded from canonical encoding (matches the ParameterDecl.name / Let.name precedent — two manifests with identical exports but different display strings hash identically); `description` and `manifestSignature` are metadata edges/fields excluded from canonical encoding (matches the Provenance precedent). The `export` list is positionally ordered in the canonical encoding (matching ProductType.fields), and each export's `declaredEffects` is encoded as a lex-sorted multi-hash list (matching FunctionType.effects — set semantics). ModuleManifest is a passive declaration: it has no runtime evaluation semantics (it is not an expression, no Application targets it). Consumers reference exports by the export's target hash directly via N-019 NodeRef; the manifest serves as a verifier-time certification of the export contract plus the natural granularity for signing, distribution, and discovery. See [Q-043](../open-questions.md#Q-043) and [`proposals/cross-store-federation.md`](../proposals/cross-store-federation.md) for the full mechanism. |

ModuleManifest does not introduce a name-based identity layer or an import system. ADR-003 was amended on 2026-05-28 to clarify that its no-import-system stance specifically rejects name-based identity, import-path resolution, visibility annotations, and version constraints — and does not reject content-addressed grouping primitives. ModuleManifest is exactly such a primitive: its identity is the hash of its export set, references to its exports remain hash-based, every reachable node remains callable, and a modified export set produces a different manifest hash. What it adds is a verifier-certified contract (each export's declared effects match its closure exactly), a coherent signing surface ([Q-006](../open-questions.md#Q-006) trust integration), and a distribution unit for ecosystem development. Cross-store resolution of NodeRef.target through a `NodeResolver` chain is the other half of [Q-043](../open-questions.md#Q-043); it is a runtime extension that does not change any node category's encoding.
...
- [`proposals/implemented/nested-recursive-types.md`](../proposals/implemented/nested-recursive-types.md) — RecursiveProjection (N-048) nested-recursive value-construction mechanism
...
- [`proposals/implemented/error-recovery.md`](../proposals/implemented/error-recovery.md) — extends node algebra with Attempt (N-047)
- [`proposals/implemented/nested-recursive-types.md`](../proposals/implemented/nested-recursive-types.md) — extends node algebra with RecursiveProjection (N-048)

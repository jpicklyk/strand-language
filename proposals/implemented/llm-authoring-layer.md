...
**Status:** Draft proposal (step 1 partially implemented 2026-05-24 — see implementation note below; steps 2-4 pending)
**Date:** 2026-05-24
...
> **Implementation note (2026-05-24).** Three sub-slices of step 1 have landed. **(A) Layer A grammar + parser + dag-json emitter** with full coverage of the implemented node algebra. New `:authoring` Gradle module under `authoring → core` with `LayerAGrammar` (42 codes spanning Layer 1 through Layer 7: literals, types, lambda/application/let/varref/NodeRef, type abstractions, effects + capability scope, foreign nodes, match + four pattern variants, fixpoint, product/sum values, recursive types, handlers, state machines + three EventStream variants + Transition, schemas + invariants; only N-030 Name and N-031 Provenance are unmapped because they have no current corpus usage). Variant-bearing categories use a `CodeSchema.discriminator` field so one Layer A code maps to each Pattern.kind / EventStream.streamKind variant. `LayerAParser` is a generic line-based tokenizer that accumulates multi-error reports; `DagJsonEmitter` performs per-code arg-shape validation + canonical dag-json assembly. The `Authoring.compileToDagJson(text)` facade is the agent-facing entry point. **(B) 33 corpus programs ship in both forms** (01-13, 15, 17-19, 21-23, 25, 27, 31-32, 35-37, 41-42, 47, 50-52) — representative coverage across every implemented feature. `LayerARoundTripTest` asserts hash-equal canonical stores for each pair. **Measured byte compression: 3.40× total across the 33-program subset** (51413 → 15083 bytes; per-program range 2.82× state machine — 4.40× match), within the §6 projection-only estimate of 2-4× and leaning toward the high end. **(C) Layer C minimal elaboration** — `Elaborator` ships effect-closure inference for Lambda. When a Layer A `LAM` node omits its optional `effects` arg, the elaborator walks the body's author-id graph and aggregates effects from reachable Application sites (closure rules mirror `Verifier.nodeClosures`: Application releases the function's declared effects + closures of fn/args; Let unions value/body; Match unions scrutinee + every case body; Handler does the subtraction rule; Lambda/ForeignNode construction releases nothing). `Authoring.compileWithElaboration(text)` is the elaborate-then-emit entry point; `strand author <file> --elaborate` is the CLI flag. End-to-end demonstration in `corpus/layer-a/elaborated/17-elided-effects.layer-a` — without `--elaborate` the verifier rejects with `UncoveredEffects`; with `--elaborate` the inferred `[timeFx]` is filled in and the program verifies cleanly. The other three Layer C inference cases (`Lambda.paramType` from call-site contexts, `Application.typeArguments` from value-argument types, `Application.effectInstances` defaulting from capability context) require type information the simple author-id graph doesn't carry and are deferred. **Step 1's remaining pieces:** the constraint-grammar derivation for Layer B (§5.2); the three other Layer C inference cases; per-corpus Layer A translations beyond the 33-program subset (~23 more programs; mechanical given the grammar is complete). The full Q-034 resolution remains gated on the Phase 1 / Q-021 measurements described in §7.

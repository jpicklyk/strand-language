...
| `corpus/50-positive-int-schema-pass.json` | PositiveInt schema accepts IntLit(5) | Small |
| `corpus/51-positive-int-schema-fail.json` | PositiveInt schema rejects IntLit(-3); test asserts violation | Small |
| `corpus/52-non-empty-list-schema-pass.json` | NonEmptyList over recursive list type, accepts Cons | Small |
| `corpus/53-non-empty-list-schema-fail.json` | NonEmptyList rejects Nil | Small |
| `corpus/README.md` | Per-program descriptions | Small |
...
| `impl-kotlin/CLAUDE.md` | Layer 7 step 1 status; new schema module documented; updated "Deferred to later layers" row | Small |
...
5. **Documentation pass** — `impl-kotlin/CLAUDE.md`, `node-algebra.md` prose addition, INDEX `Last revised`.

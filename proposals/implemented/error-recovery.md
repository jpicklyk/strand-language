...
5. **The § 8 builtin-contract hygiene follow-up (`BuiltinContractViolation`) shipped as a same-day follow-up commit rather than in the slice** — deliberately deferred at integration to protect the green state, then landed separately: a new uncatchable `InterpretError.BuiltinContractViolation(at: NodeId?, target, detail)` (nullable `at` per the `ResourceExhaustion` precedent — VM opcodes carry no NodeIds), translating `IllegalArgumentException` strictly at the six `builtin.invoke` boundaries (four interpreter paths — `applyValue`, the higher-order and standard `applyForeign` paths, `applyValueToArgs` — and the VM's `invokeCallable` and `applyClosure` VmForeign branches). `Int.Div`/`Int.Mod`/`Math.Mod` by zero now surface structured and attributable; Attempt does not observe them (`isCatchable = false`, proven by a TRY-propagation test) and interpreter/VM parity on variant, target, and detail is test-asserted.
...
During integration a pre-existing prelude inconsistency was discovered (present before this work): the prelude `fsRead`/`fsExists` ForeignNodes carry an `ArgRef(0)` effect projection against the parameterless `readFx` EffectCategory, so authoring a prelude-`fsRead` program fails with `ProjectionSourceArityMismatch`. Investigation widened the diagnosis to all six Q-039-projected prelude entries (the `writeFx` and `connectFx` categories were equally parameterless), and the fix landed the same day in commit `c571cd0`: the three categories now carry their E-006/E-007/E-001 parameters, with end-to-end admission, interpreter, and registry-consistency tests closing the coverage gap.

...
**Document:** `proposals/implemented/interpreter-resource-limits.md`
...
> **Implementation note (2026-05-27).** Landed in the Kotlin/JVM reference implementation as designed. `core/EvaluationLimits.kt` ships the `EvaluationLimits` data class with the eight fields and `DEFAULTS` / `PERMISSIVE` companions; `ExhaustionKind` enum covers all seven variants. `core/Json.kt` gains a `parse(text, limits)` overload, a public `validateJsonDepth(text, maxDepth)` helper (linear `{`/`[` brace counter ignoring string contents), and a sealed `IngestError` with `Malformed(message)` and `ResourceExhaustion(kind, current, limit)` variants. Every `throw IngestError(...)` call site was migrated to `IngestError.Malformed(...)`. `InterpretError.at` was promoted to nullable to accommodate the VM's missing source-mapping; `InterpretError.ResourceExhaustion(at: NodeId?, kind, current, limit)` is the new variant. `Interpreter.EvalCounters` is a public `data class` with `var steps, var currentDepth, var allocated, val startNanos`; threaded through every `eval` / `apply*` / `evalMatch` / `evalProductValue` / `evalSumValue` / `evalProductFieldGet` / `tryMatch` / `evalEffectInstances`. Per-step guards live at the head of every `eval`: step counter, depth counter (`currentDepth++` / `currentDepth--` in try/finally), wall-clock sample every `wallClockSampleEvery` steps. Allocation counter (`allocV`) wraps every Value-construction site in the dispatch path. `Vm.kt` adopts the same guard shape using `frames.size` for stack depth and a local `bumpAllocation` helper at PUSH_*/MAKE_*/PRODUCT_NEW/SUM_NEW/EQ/SUM_CASE_IS sites; the VM-internal `VmResourceExhaustion` is mapped to the shared `InterpretError.ResourceExhaustion` at the `runLoopMapped` boundary. `StateMachineRuntime.runMachine` / `runGroup` / `resume` thread `limits` and reuse one `EvalCounters` across events (one logical run, one budget for sync; one per actor for async). `HaltReason` promoted from enum to sealed class with the new `ResourceExhaustion(kind, atEventIndex)` variant. `SchemaChecker` takes an optional `limits: EvaluationLimits = DEFAULTS`. CLI flags `--max-steps`, `--max-stack-depth`, `--max-allocated-values`, `--wall-clock-ms`, `--max-json-depth`, `--max-node-count`, `--max-ingest-bytes` work on `run`, `machine`, and `group` subcommands. Test coverage spans 9 ingest tests in `JsonIngestLimitsTest`, 7 interpreter tests in `InterpreterLimitsTest`, 3 cross-backend equivalence tests in `VmLimitsTest`, 4 state-machine runtime tests in `StateMachineRuntimeLimitsTest`, plus 2 corpus tests in `CorpusLimitsTest` over corpus programs 71 (Fixpoint with no base case) and 72 (300-deep Application chain). Full `gradle test` clean — 861 tests pass with zero regressions. Hash invariance preserved across all 68 pre-Q-040 corpus programs (the proposal touches no node encoding).
>
> **Deviations from the proposal worth recording.** (1) **Corpus 72 depth.** The proposal § 9 sketch indicates "a deeply-nested Application chain large enough to exceed `maxStackDepth=4096`." Empirically the canonical hasher's `walk` traversal is itself recursive over Application's function/argument tree, so an 800+-deep chain overflows the JVM stack during hashing (before the interpreter even runs). The corpus 72 ships at depth 300, and `CorpusLimitsTest` uses `maxStackDepth = 100` to exercise the host-side cap below the JVM native stack tolerance — the proposal's "stack-depth counter fires before native stack exhaustion" contract holds, just at lower numeric values than the original example. The Hasher's own recursion is a candidate for iteration-rewrite in a Q-040 follow-up. (2) **Scenario 6 (WallClock) testing.** The proposal § 7 specifies a productive-non-termination Fixpoint with `wallClockBudgetMillis = 100` raising `WallClock`. In the tree-walking interpreter, each productive recursion grows `currentDepth` by a constant amount per recursion, so the stack-depth counter often races ahead of wall-clock. `InterpreterLimitsTest` exercises both kinds (WallClock or StackDepth) for portability across host speeds; on a fast machine WallClock wins. The proposal's intent — a recoverable error rather than a hang — is satisfied either way. (3) **Corpus 71 testing.** Same as (2): the proposal expects `Steps` to fire first, but on small `maxSteps` budgets the stack-depth counter can fire first because each recursion adds ~3-4 eval steps. The test accepts both. (4) **`Value.UnitV` is shared.** The `allocV` helper bumps a counter for every reachable Value constructor, EXCEPT `Value.UnitV`, which is a singleton — sharing one instance has zero per-instance memory cost, and counting every unit-return would dominate the signal under `maxAllocatedValues`. The proposal § 4.3 text refers to "each Value.* constructor"; we preserve this for non-singleton values. (5) **`EvalCounters` visibility.** The proposal text reads "internal `EvalCounters` struct." Because `:runtime` is a separate Gradle module and Kotlin's `internal` is module-private, `EvalCounters` and the counter-threading `applyCallable` overload are promoted to `public` on the `Interpreter` class. The semantic contract is unchanged; the visibility is loosened so the runtime can share one counter across per-event applyCallable calls (the proposal's § 4.5 "one logical run, one budget" rule).

...
**Document:** `proposals/implemented/credential-isolation.md`
**Status:** Resolved (implemented 2026-05-27)
**Date:** 2026-05-26
**Concerns:** [`design/security-model.md`](../../design/security-model.md), [`decisions/ADR-005-foreign-nodes.md`](../../decisions/ADR-005-foreign-nodes.md), [`proposals/implemented/agent-native-capabilities.md`](agent-native-capabilities.md) § 4.4 (credential provider design), [`proposals/implemented/agent-native-vector-stores.md`](agent-native-vector-stores.md) § 4.2, [Q-042](../../open-questions.md#Q-042), [`security-index.md`](../../security-index.md) § Finding 4
**Scope:** Small-medium

> **Implementation note (2026-05-27).** Landed in the Kotlin/JVM reference implementation as designed, grafted onto the Q-040 `EvaluationLimits` carrier that landed earlier the same day. `interpreter/Credential.kt` ships the wrapper with an `internal constructor(value, provider, credentialKey)`, redacted `toString` (`[REDACTED:<provider>:<credentialKey>]`), explicit `reveal()`, and reference-identity `equals` / `hashCode`. `interpreter/CredentialScrubber.kt` ships the process-global `@Volatile var registry: Map<String, String>` with copy-on-write `register(Credential)` (skips blank / sub-8-character values), `scrub(text: String)` (linear in `text.length × registry.size`; fast path when registry is empty), and `internal resetForTesting()` for `@AfterEach` isolation. `interpreter/CredentialProvider.kt` updated to return `Credential?` from `resolve` and `apiKey`; the three default implementations (`EnvCredentialProvider`, `StaticCredentialProvider`, `InMemoryCredentialProvider`) mint a `Credential` and call `CredentialScrubber.register` before returning. Five per-provider files migrated with seven total `.reveal()` call sites in production code (one for Anthropic header; two each for OpenAI and Gemini covering generate + embed; one each in Pinecone and Chroma `standardHeaders` builders) — Pinecone and Chroma also got their handle types' `apiKey: String` → `apiCredential: Credential` (or `Credential?` for Chroma's optional-auth case). `interpreter/ResourceTable.kt`'s `IoFailure` constructor now scrubs `detail` at construction and exposes both `detail` (scrubbed) and `unscrubbedDetail` (raw) — every existing `throw IoFailure(...)` call site benefits automatically. `core/ErrorVerbosity.kt` ships the three-variant enum (`Redacted` default; `Full` opt-in with runtime-entry warning; `RedactedWithKindOnly` strict); `core/EvaluationLimits.kt` grew an `errorVerbosity: ErrorVerbosity = Redacted` field with `PERMISSIVE` preserving `Redacted` (resource limits, not credential leakage). `Interpreter.translateIoFailure` is the centralized helper that materialises `InterpretError.IoFailure` per the active verbosity; all four `IoFailure` catch sites in `Interpreter.kt` (two on the standard dispatch path, one each in the higher-order and `applyValueToArgs` paths) route through it. The `ForeignDispatcher` invocation path also gained an `IoFailure` catch wrapping the dispatcher call — defensive coverage so a dispatcher that throws IoFailure (none ship today) still routes through scrubbing. CLI flag `--error-verbosity {redacted|full|kind-only}` works on `run`, `machine`, and `group` subcommands (parsed in the shared `parseLimits` block) with the runtime-entry warning printed once when `full` is selected. Test coverage spans `CredentialTest` (10 cases — scenarios 1, 2, 8), `CredentialScrubberTest` (14 cases — scenarios 3, 9, 10 plus boundary/multi-credential coverage), `ErrorVerbosityTest` (5 cases — scenarios 6, 7 plus PERMISSIVE/DEFAULTS sanity), and Q-042-specific additions to each per-provider test (Anthropic gets scenarios 4 + 5; OpenAI / Gemini / Pinecone / Chroma get scenario 4) — 34 new tests total. Full `gradle test` clean — 895 tests pass with zero regressions across the 861 pre-Q-042 corpus. Hash invariance preserved across all 68 corpus programs (the proposal touches no node encoding).
>
> **Deviations from the proposal worth recording.** (1) **Dispatcher path also catches IoFailure.** The proposal § 4.5 specifies threading verbosity through the existing `applyForeign` IoFailure catch. The implementation also wrapped the `foreignDispatcher.dispatch(...)` calls (one in each of three call sites: `applyCall`'s Value.ForeignFn branch, `applyForeign`, and `applyValueToArgs`) in IoFailure catches that route through `translateIoFailure`. Defensive coverage: no existing dispatcher (the runtime supervisor in `Supervision.kt`) throws IoFailure today, but the catch makes a future dispatcher safe-by-default and made the test infrastructure simpler — `ErrorVerbosityTest` uses a test-only `ForeignDispatcher` to provoke IoFailures without registering temporary builtins into `Builtins.lookup` (the registry is `private val`). (2) **`Credential` constructor visibility.** The proposal § 4.1 says "the `internal` constructor restricts construction to the `interpreter` module — only `CredentialProvider` implementations can mint a `Credential`. Test code constructs through `StaticCredentialProvider` / `InMemoryCredentialProvider`, which call the internal constructor." Tests in the same module (`:interpreter`) can in fact call the internal constructor directly, and `ErrorVerbosityTest` and the updated `VectorRefinementTest` exercise this — minting a `Credential` directly with a controlled raw value to seed the scrubber without going through a provider. The semantic intent — *external* host code cannot mint Credentials — is preserved by Kotlin's module-internal visibility. (3) **`PineconeIndexHandle` and `ChromaCollectionHandle` schema change.** The proposal § 4.2 specifies one `.reveal()` per provider at the header builder, and the LLM providers (Anthropic, OpenAI, Gemini) fit that mold trivially. The vector providers (Pinecone, Chroma) cache the credential in the resource-table handle for later use across multiple operations against the same index/collection. The migration changed the handle field types from `String` / `String?` to `Credential` / `Credential?`, with the single `.reveal()` per provider living in the `standardHeaders` helper that every operation funnels through. This matches the proposal's spirit (single auditable surface per provider) but represents a small handle-shape change that test code constructing handles directly (`VectorRefinementTest`) needed to update — two call sites, mechanical. (4) **`InterpretError.IoFailure` retains its single-`detail` shape.** The proposal § 4.5 reads "The `IoFailure` constructor stores both `detail` (scrubbed) and `unscrubedDetail` (raw) — the unscrubed field is read only under the `Full` verbosity and is otherwise inert." The implementation keeps `InterpretError.IoFailure(at, kind, detail: String)` exactly as Q-040 left it — the dual-field storage lives on the runtime `IoFailure` exception in `ResourceTable.kt` (the `RuntimeException` subclass thrown by builtins), not on `InterpretError.IoFailure` (the verifier-translated form). The `translateIoFailure` helper consults the verbosity and picks the right source string before constructing the `InterpretError.IoFailure`. This keeps `InterpretError.IoFailure` immutable in shape (no Q-042 visible field added to the public error sum) while still satisfying the verbosity contract. (5) **`unscrubedDetail` → `unscrubbedDetail`.** The proposal § 4.5 uses the spelling `unscrubedDetail`; the implementation uses `unscrubbedDetail` (two b's, conventional English) for the property name. Pure naming preference. (6) **No new corpus programs.** The proposal § 9 marks corpus optional ("If you want a smoke-test corpus that exercises the verbosity flag at the CLI, that's a nice-to-have but not required"). None ship — credential handling is runtime policy, not graph well-formedness, and the test suite already exercises the verbosity flag end-to-end at the interpreter level.

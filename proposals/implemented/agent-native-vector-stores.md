...
**Document:** `proposals/implemented/agent-native-vector-stores.md`
**Status:** Resolved (Phase 1 implemented 2026-05-26 in two parallel-worktree commits — `10c13e0` Q-038 work + `3c8271b` merge with Q-037 / shared-infrastructure conflict resolution)
**Date:** 2026-05-26
...
> **Implementation note (2026-05-26, Phase 1 landed).** Per-provider vector-store ForeignNodes for Pinecone and Chroma under operation-shaped effect categories E-037 Vector.Read{provider, store} + E-038 Vector.Write{provider, store}. Read / Write split mirroring Filesystem.Read / Filesystem.Write (E-006 / E-007). Twelve builtins total: `Pinecone.Index.{Open, Close, Upsert, Query, Delete, Fetch}` and `Chroma.Collection.{Open, Close, Add, Query, Delete, Get}`. 24 new tests across `BuiltinsPineconeTest` (10), `BuiltinsChromaTest` (10), `VectorRefinementTest` (4); all pass. Full `gradle test` clean. Shipped: `PineconeProvider.kt` / `ChromaProvider.kt` (stateless request/response with injectable `VectorHttpTransport` — `JdkHttpTransport` default, `InMemoryHttpTransport` for tests), `VectorTypes.kt` (shared `UpsertItem` / `QueryRequest` / `QueryHit` shapes + `Metric` sum), `pinecone_index` / `chroma_collection` ResourceTable kinds, twelve prelude entries plus `vectorReadFx` / `vectorWriteFx`, system-prompt Vector stores section, corpus 68 verify-only Pinecone upsert + query exemplar (renamed during merge from `67-*` because Q-037 had also claimed 67). **Deviations from the proposal worth recording:** (1) Surface types use `bytesT` placeholders for opaque Resource handles and for structured payloads (config / items list / QueryRequest / QueryHit list) — matches the Map.* opaque-handle convention. A future parametric `Resource<K>` / `Record<...>` primitive would be a generalization. (2) Corpus 68 is verify-only: runtime requires HTTP mocking that `CorpusTest` doesn't provide; the BuiltinsPineconeTest integration test fills the end-to-end runtime gap. The RAG demo corpus is deferred to Phase 3. (3) At parallel-worktree merge time the `CredentialProvider` interface was unified with [Q-037](agent-native-capabilities.md): single trait exposing both `resolve(provider, credentialKey): String?` (this proposal's general form, used by Pinecone/Chroma) and `apiKey(provider): String?` (Q-037's convenience). (4) Manhattan metric is rejected by both providers at the wire-name translation level because neither natively supports it; this wasn't called out in the proposal but is honest to provider capability. Phases 2 (pgvector + FAISS) and 3 (Weaviate + Qdrant + RAG-demo corpus) remain.

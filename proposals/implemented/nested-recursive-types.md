...
- **Migrating corpus 66 / `Json.Parse` / `Json.Stringify` to the precise model** — *resolved 2026-06-13 as [Q-069](../../open-questions.md#Q-069), [`proposals/implemented/json-precise-model.md`](json-precise-model.md).* The `Json.*` builtins and the agent-facing `Sig.JsonValue` tower migrated in place to the precise model; corpus 66 was retired-and-replaced (rather than kept as a historical exemplar — the spliced model had no remaining consumer) with a precise round-trip demonstrator. The `UniqueKeyJsonObject` blessed-schema re-expression on the precise model remains deferred there with a recorded unblocker.

...
**Document:** `proposals/implemented/actor-runtime-stream-bridge.md`
**Status:** Implemented 2026-06-03 (Kotlin/JVM reference implementation, core slice)
**Original date:** 2026-06-03
**Concerns:** [`design/state-machines.md`](../../design/state-machines.md) (the push-based actor runtime, `External` EventStream, `MachineGroupHandle.externalInputs`, `OverflowPolicy`, `StreamBus`, `EventRecorder`), [`proposals/implemented/streaming-async-io.md`](streaming-async-io.md) § 8 (the Q-045 pull primitive this builds on, and the deferral that names this question), [`design/effects-and-capabilities.md`](../../design/effects-and-capabilities.md) (E-004 `Network.Receive` vs. the runtime-implicit E-029 `StateMachine.Receive`), [`decisions/ADR-003-content-addressing.md`](../../decisions/ADR-003-content-addressing.md) (additive field versioning), [`decisions/ADR-004-effects-as-edges.md`](../../decisions/ADR-004-effects-as-edges.md) (mandatory effect declarations), [`decisions/ADR-007-state-machines.md`](../../decisions/ADR-007-state-machines.md) (atomic pure transitions), [Q-046](../../open-questions.md#Q-046), [Q-044](../../open-questions.md#Q-044) (the harm bound this preserves), [Q-009](../../open-questions.md#Q-009) (event ordering / nondeterministic merge), [Q-015](../../open-questions.md#Q-015) (backpressure), [Q-040](../../open-questions.md#Q-040) (`streamReceiveTimeoutMillis`), [Q-041](../../open-questions.md#Q-041) (sandbox at open), [Q-042](../../open-questions.md#Q-042) (error redaction)
**Scope:** medium

## Implementation note (2026-06-03)

The core slice landed across the node-algebra, hashing, verifier, and runtime layers; no new node category, ADR, or effect-category identifier; every existing program hash is preserved. Changes:

- **`core/Node.kt`** — `EventStream` gains the optional `source: NodeId? = null` edge. **`core/Json.kt`** parses it via the existing `optionalRef` idiom; **`core/NodeChildren.kt`** and **`core/NodeTranslate.kt`** thread it as an optional child edge (so reachability, hashing, and federation NodeId translation cover it).
- **`hashing/CanonicalEncoder.kt`** — `encodeEventStream` appends `source` as a trailing hash field gated on non-null, in both the base-only and the slice-3.1/3.6 branches. When `source == null` the bytes are identical to the pre-Q-046 encoding; a source-only stream encodes to three fields (a length no pre-Q-046 EventStream produces) and the trailing field is a byte string, so it never collides with the trailing uint of the five-field form. **`hashing/Hasher.kt`** walks the `source` child. `CategoryTag` unchanged (no new category).
- **`verifier/Verifier.kt`** — `resolveEventStream` calls a new `validateStreamSource` helper when `source` is non-null, enforcing: `External` only, `eventType == Bytes`, `BlockProducer` overflow, `source` resolves (via a new `resolveForeignTarget` walker mirroring `resolveProjectedForeignNode`) to an `Application` of a registered opener target, and the opener declares its expected semantic effect. A top-level `STREAM_OPENER_TARGETS` registry maps the three `*.CreateStream` targets and `Net.Connect` to `LLM.Generate` / `Network.Connect`. **`verifier/VerifyError.kt`** adds the five variants `StreamSourceOnNonExternal`, `StreamSourceNotAnOpener`, `StreamSourceEffectMismatch`, `StreamSourceTypeMismatch`, `ByteStreamSourceRequiresBlockProducer`. `WellKnownEffect` is **not** extended (the bridge's transport effects are real value-level effects, not runtime-implicit ones — matching the Q-045 § 5 decision).
- **`runtime/MachineGroup.kt`** — `sourceBoundExternalStreams()`, the group-start soundness gate `validateStreamSourceCoverage()` (the opener's declared effect category names ∪ `Network.Receive` must be covered by `group.capabilities`, checked by category name since the feeder's `Network.Receive` has no graph node to key on), and the new `MachineGroupValidationError.ExternalStreamSourceEffectUncovered`. **`runtime/ExternalStreamFeeder.kt`** (new) is the feeder coroutine: it runs each blocking `Stream.Receive` on `Dispatchers.IO`, pushes chunks through the stream's existing `OverflowDispatcher`, registers/halts as the stream's producer via `StreamBus.producerSpawned`/`producerHalted` (so EOF closes the channel and the consumer halts), closes the handle exactly once in a `finally`, and dispatches on the handle kind to the LLM vs. socket receive/close builtins. **`runtime/StateMachineRuntime.kt`** `runGroup` calls the coverage gate after `validateTopology`, then in a new Pass 2c opens each bound `source` once (via an `Interpreter` bound to the group's store, under the group's capabilities) and launches one feeder per stream, joining the feeder jobs into the handle's job list.
- **Tests** — `runtime/BridgedStreamTest` (8 cases: the five verifier rejections, happy-path admission, the adversarial group-start coverage gate, and an end-to-end loopback-socket bridged drain asserting both the accumulated payload and deterministic per-instance replay through `runMachine`). Verify-only corpus 84 (`84-bridged-stream.json`) in `CorpusTest` + `CorpusHashingTest`. Full suite: **1165 tests green** (1157 baseline + 8).

Deviations and decisions worth recording:

1. **Group-start coverage is checked by category *name*, not NodeId.** The feeder's `Network.Receive` drain has no `EffectCategory` node in a bridge program (the program declares E-029 on the machine and E-035/E-001 on the opener; no drain appears in any Lambda). The gate therefore resolves the granted capability NodeIds and the opener's declared EffectDecls to category names and checks name coverage. This confirms the category is granted — sufficient to keep `CapabilityScope` revocation biting — while the concrete socket is independently constrained by the Q-041 sandbox at open. NodeId-precise refinement matching at the gate is unnecessary for soundness and is not implemented.
2. **`source` is required to be an `Application` directly** (not a bare `VarRef`/`Let`/`NodeRef` to one). The proposal § 4.1 allowed "an `Application` of (or a bare reference to)" an opener; the foundational slice narrows to the direct-`Application` form (matching the worked example), so the runtime reads the opener's `effectInstances` straight off `store.get(source)`. The callee *inside* the Application may still be reached through `VarRef`/`Let`/`NodeRef` (the `resolveForeignTarget` walker handles those). A bare-reference `source` is a deferred ergonomic extension.
3. **Layer A `source` grammar deferred.** Corpus 84 is authored as canonical dag-json; no `ESE`-code extension for the `source` arg ships in this slice, matching how `effectProjections` and the ModuleManifest `MFT`/`MEX` codes were deferred. Corpus 84 is therefore excluded from `LayerAReverseRoundTripTest` (which uses an explicit program list, not auto-discovery).
4. **Decoded events, group-level replay, multiple bridged streams per machine, streaming send, and process sources remain deferred** per § 8. The raw-`Bytes` slice cannot carry a typed failure case, so a transport failure surfaces to the machine as the same channel closure as a clean EOF; the cause is retained on the feeder's `failureKind` for diagnostics.
...
- [`proposals/implemented/streaming-async-io.md`](streaming-async-io.md) — Q-045 § 8 names this bridge and its four sub-questions; the pull primitive (`Stream.Receive` / `Close`, `streamReceiveTimeoutMillis`) the feeder calls

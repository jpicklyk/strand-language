...
**Document:** `proposals/implemented/agent-native-capabilities.md`
**Status:** Resolved (Phase 1 implemented 2026-05-26 in two parallel-worktree commits — `4990a45` Q-037 work + `3c8271b` merge with Q-038 / shared-infrastructure conflict resolution)
**Date:** 2026-05-26
...
> **Implementation note (2026-05-26, Phase 1 landed).** Per-provider LLM ForeignNodes for Anthropic, OpenAI, Gemini under operation-shaped effect categories E-035 LLM.Generate{provider, model} + E-036 LLM.Embed{provider, model}. 23 files / +3340 lines / 34 new tests across `BuiltinsAnthropicTest` (5), `BuiltinsOpenAITest` (4), `BuiltinsGeminiTest` (4), `JsonSchemaProjectionTest` (15), `ToolDispatchLoopTest` (2), `ProviderRefinementTest` (4). All tests pass; full `gradle test` clean. Shipped: `CredentialProvider.kt` (env-var default, with `StaticCredentialProvider` / `InMemoryCredentialProvider` test helpers), `AnthropicProvider.kt` / `OpenAIProvider.kt` / `GeminiProvider.kt` (stateless request/response with injectable `LlmHttpClient`), `LlmProviders.kt` (shared unified shapes), `JsonSchemaProjection.kt` (irreducible TypeExpr subset → JSON Schema), `VerifyError.ToolParamTypeUnsupported` variant, six `strand-builtin:` entries in the higher-order registry (tool-loop driven, bounded at 10), `llm_conversation` ResourceTable kind registered, six prelude entries (`anthropicGenerate` etc. plus `llmGenerateFx` / `llmEmbedFx`), system-prompt section with Layer A emission example, corpus 67 verify-only state-machine demonstrator. **Deviations from the proposal worth recording:** (1) **Resolved 2026-05-26 via N-044 ToolDef.** The `ToolParamTypeUnsupported` verifier walk originally shipped as a `VerifyError` variant + `JsonSchemaProjection` helper without a structural enforcement site, because ToolDef was a runtime `ProductV` (per proposal § 5's option (b)) rather than a graph node. The principled fix landed via N-044 ToolDef: ToolDef is now a first-class graph node (Path B from the architectural deep-dive), the verifier statically projects every ToolDef's parameterSchema.valueType to JSON Schema at admission, and ToolParamTypeUnsupported fires by construction on any unsupported variant. `JsonSchemaProjection` moved from the interpreter module to the verifier module so the rule can run without a circular dependency. Corpus 67 was rewritten with a real `Schema`-backed ToolDef; the previous `bytesT` placeholder for `tools` is replaced with a structurally-checked tool declaration. New `Value.ToolDefV` runtime variant carries the resolved schema NodeId + implementation callable; the LLM.Generate tool-dispatch path looks up the SchemaType via the verifier's `nodeTypes` map (carried by the interpreter through a new `Builtins.verifierNodeTypes` singleton) and projects at dispatch time. New verifier errors: `ToolImplementationOverPolymorphic`, `ToolImplementationNotAFunction`, `ToolImplementationArityMismatch`, `ToolImplementationParameterTypeMismatch`. New Layer A code `TLD <name> <description> <parameterSchema> <implementation>`. (2) Prelude `llmGenerateT` / `llmEmbedT` use `bytesT` placeholders for the GenerateRequest / EmbedRequest products — the Strand-side shapes are agent-chosen towers (List<Message>, List<ToolDef> with nested sums) that don't fit a single monomorphic FunctionType. Same surface-type pattern Map.* uses; agents declare explicit FNT + FRN at use sites for precise typing. (3) Anthropic embeddings stub throws `IoFailure("anthropic-embed-not-supported", ...)` recommending Voyage AI — a `VoyageProvider` is a clean follow-up slot. (4) `Builtins.llmHttpClient` is a single global `@Volatile` mirroring `clock` / `random` / `credentialProvider`; per-provider HTTP-client injection is a deferred refinement if production deployments need divergent timeouts/retries. (5) At parallel-worktree merge time the `CredentialProvider` interface was unified with [Q-038](agent-native-vector-stores.md): single trait exposing both `resolve(provider, credentialKey): String?` (Q-038's general form) and `apiKey(provider): String?` (Q-037's convenience, defaults to `resolve(provider, "api_key")`). `StaticCredentialProvider` (Q-037 test helper) and `InMemoryCredentialProvider` (Q-038 test helper) both preserved. Phase 2 (agent-pattern documentation + reference corpus programs) remains. **Resolved 2026-05-26 via N-045 ResponseSchemaSpec.** The responseSchema follow-up flagged in the N-044 commit landed as a one-edge wrapper graph node mirroring the N-044 ToolDef pattern. `GenerateRequest.responseSchema` now accepts a `Value.ResponseSchemaSpecV` runtime carrier — produced when the interpreter evaluates a `Node.ResponseSchemaSpec(schema)` — instead of the `Option<JsonValue>` JSON Schema tower. Spec § 3.7's "translate the Schema's valueType into JSON Schema and pass it to the provider" is now structural: the verifier statically projects `schema.valueType` via `JsonSchemaProjection.project()` at the wrapper's admission and raises the new `ResponseSchemaTypeUnsupported` for unsupported variants (FunctionType / ForallType / unbound TypeParameter), so the verifier's static guarantee covers both Schema-bearing positions on `GenerateRequest`. `Builtins.parseGenerateRequest` looks up the wrapper's `parameterSchemaId` (the schema NodeId) through the existing `Builtins.verifierNodeTypes` singleton (the same mechanism N-044 uses for tool parameter schemas) and submits the projected JSON Schema to the provider library in the same `JsonElement?` shape it previously received from the JsonValue extraction. New Layer A code `RSC <schema>`. New `Value.ResponseSchemaSpecV(self, schemaId)` runtime variant. Category tag 45. The N-045 wrapper is thinner than ToolDef (no metadata, no implementation) — its single semantic is "carry a Schema reference into the responseSchema field position".

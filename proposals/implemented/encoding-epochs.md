...
**Status:** Implemented at the policy level (adopted 2026-06-13; epoch-2 execution deferred — see the Implementation note)
**Date:** 2026-06-11
...
**Epoch-2 execution is deferred.** It rides the Q-053 design work and bundles optional-field normalization and the Q-049 `bound` decision per the § 2 charter; nothing in the charter is in force, and the charter's authoritative record is § 2 of this document.

On § 4 scenario 5 (mixed-epoch rejection): with only epoch 1 in existence there is no second epoch to reject live. The fail-closed path the scenario relies on — a NodeRef resolved against a peer store whose content hashes differently than claimed — is the federation integrity check, already exercised by `CorpusFederationTest`'s corpus-78 case (a resolver serving mismatching content halts with `NodeResolverIntegrityViolation`) and the narrower `FederatedProgramTest` / `NodeResolverTest` unit cases; a cross-epoch fetch fails on exactly that hash mismatch. No fake epoch-2 fixture was manufactured to restate it.

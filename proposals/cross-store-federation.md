...
**Status:** Partially implemented — N-046 ModuleManifest (step 3b) landed 2026-05-28; the step-3a federation runtime *core* (cross-store admission with bound-node re-basing + integrity, threaded through the verifier and interpreter) landed 2026-05-29. Remaining for step 3a: VM / StateMachineRuntime / SchemaChecker threading, the external-hash NodeRef ingest form, the CLI flags + registry subcommands, and corpus 76/77/78. See § 10 Implementation status.
**Date:** 2026-05-28
...
- **External-hash NodeRef ingest form** — JSON ingest currently resolves a `NodeRef` target only as a local author id, so authoring an app that references a *peer's* hash needs a new ingest form (e.g. a `targetHash` field producing a canonical external `Node.NodeRef` directly, since a `RawNodeRef` targets a local id and a Canonical NodeRef cannot sit in the raw store). The verifier/interpreter cross-store tests build the federated program programmatically to sidestep this.
- **CLI** `--peer-store` / `--registry` / `--no-cache` / `--strict-integrity` flags and `strand registry resolve|put|list` subcommands; **corpus 76 / 77 / 78**; `CliFederationTest`.

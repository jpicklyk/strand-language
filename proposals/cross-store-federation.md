...
**Status:** Implemented (Kotlin/JVM reference implementation). N-046 ModuleManifest (step 3b) landed 2026-05-28; the step-3a federation runtime completed 2026-05-29 — cross-store admission with bound-node re-basing + Merkle integrity (`translateNodeIds` + `fetchAndAdmit`), the `targetHash` external-hash NodeRef ingest form, and the optional `resolveTarget` callback threaded through every backend (verifier, interpreter, bytecode Lowerer, StateMachineRuntime, SchemaChecker); `strand run/verify/machine/group --peer-store` plus `--no-cache`/`--strict-integrity`; the `strand registry resolve|put|list` subcommands; and corpus 76 (composition), 77 (name registry), 78 (integrity violation). A latent interior-hash-dedup aliasing bug in `fetchAndAdmit` — unsafe for context-dependent (open) nodes whose standalone hash is a de-Bruijn sentinel — was fixed in the same pass. Networked resolvers, signed-manifest verification (Q-006), persistent caching, multi-version naming, async resolution, and cross-store GC remain deferred. See § 10 Implementation status.
**Date:** 2026-05-28
...
**Step 3a — federation runtime — completed 2026-05-29.** The bound-node blocker is resolved and a cross-store program verifies, runs, lowers to bytecode, drives a state machine, and schema-checks end-to-end, including from the CLI.
...
*Also landed (parts 5-6, 2026-05-29):*
- **`resolveTarget` threaded through the remaining backends.** The same optional `resolveTarget: ((Hash) -> NodeId?)?` callback (default null) now flows into `bytecode/Lowerer` (a cross-store NodeRef target is fetched + admitted before its sub-chunk is lowered; the VM runs the already-resolved `ChunkTable` unchanged), `runtime/StateMachineRuntime` (into the default interpreter for the sync `runMachine`/`resume` fold and into the `RuntimeContext` that builds per-actor interpreters for the async `runGroup` path), and `schema/SchemaChecker` (its static-eval NodeRef site and the invariant-body interpreter). A cross-store program now resolves on every backend, not just verifier+interpreter. Coverage: `CrossStoreVmTest`, `CrossStoreRuntimeTest` (sync + async), `CrossStoreSchemaTest`.
- **`--peer-store` on `machine` / `group`**, the `--no-cache` (skip the default `CachingResolver` wrapper) and `--strict-integrity` (explicit-declaration no-op — the per-target Merkle re-hash is always on) flags on every federated subcommand, and the `strand registry resolve|put|list --registry <file>` subcommands over the off-graph `NameRegistry` (with a new `Hash.fromHex` enabling `put`). **corpus 77** (name-registry resolution; the entry tracks corpus 76's live `inc` hash) and **corpus 78** (integrity violation — exercised programmatically by a `BadResolver`, since a finalized peer cannot return hash-mismatched content). `CliFederationTest` drives the real `strand` entry point on happy paths (`run`/`verify --peer-store`, `registry resolve|put`); failure paths stay out of the test because `main()` uses `exitProcess`, and the rejection logic is covered by `CorpusFederationTest`.

*Integrity-correctness fix (2026-05-29).* The original `fetchAndAdmit` deduped each fetched interior node against the local store by its standalone per-node hash. That is unsafe for context-dependent (open) nodes: `Hasher.finalize` encodes a free binder reference (a bare `VarRef`, `RecursiveSelf`, or `TypeParameter` reference) via the unbound de-Bruijn sentinel, so every such node hashes identically regardless of which binder it points at. A `VarRef` to a first parameter therefore collides with any other first-parameter `VarRef`, and deduping on that collision aliased the fetched node's binder to an unrelated local binder — shifting a de-Bruijn position and corrupting the re-base, which the root re-hash then rejected as a spurious integrity violation. corpus 76 never tripped it (its app shares no node shape with the peer); it surfaces the moment an app and a peer both contain a same-shaped `VarRef`, which is nearly every real cross-store program. The fix admits every fetched interior node fresh — within-fetch DAG sharing and reference cycles are handled by the foreign→local memo, and cross-target sharing by `fetchAndAdmit`'s leading whole-target memo, so the only cost is some duplicate leaf nodes (functionally invisible, since type equality and hashing are structural). Regression: `FederatedProgramTest` admits `inc` into an identity-Lambda app whose own `VarRef` collides.

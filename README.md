...
- **Dynamic** (tokens-per-successful-task across the agent retry loop, ten-task suite, fresh-context subagents, no caching): Strand at **6.56× Python+type-hints** total tokens, with **9/10 first-pass / 10/10 converged** vs Python's 10/10 / 10/10. Per-emission output tokens are **16% leaner than Python** (1,180 vs 1,412 across the suite); the input-token gap is driven entirely by the system-prompt overhead (10.1k vs 1.5k) and is the natural target for prompt caching. See [`evaluation/dynamic-results.md`](evaluation/dynamic-results.md) for the full per-task breakdown.

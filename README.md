...
A three-task evaluation suite under [`evaluation/`](evaluation/) measures static byte cost across baselines. Current results: Strand canonical dag-json runs at 5.82× Python+type-hints geomean; Strand Layer A density v4 runs at 0.81× geomean — below the Python+type-hints baseline on every task in the MVP, without tokenizer-side fine-tuning. Dynamic metrics (first-pass verification rate, tokens-per-successful-task across an agent's retry loop) remain to be measured and require model-API integration.
